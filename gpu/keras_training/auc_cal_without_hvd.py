...
def load_batch(batch_of_files, is_training=False):
    ...
    for filename in batch_of_files:
        img = pil.open(os.path.join('/workspace/data/chestxray/images/', filename))
        #img = pil.open('/workspace/images/', filename)
        img = img.convert('RGB')
...
print("file path is: ", os.path.dirname(os.path.realpath(__file__)))  
...
with open('/workspace/cheXnet_tf_hvd/avg_auc.log', 'w') as f:
    f.write(str(np.array(avg_scores).mean()))
print("Logged Average AUC Scores!")
