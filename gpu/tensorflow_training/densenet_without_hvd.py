...
parser.add_argument(
    '--data_dir', type=str, default='/workspace/data/xraytfrecords/',
    help='The directory where the input data is stored.')

parser.add_argument(
    '--model_dir', type=str, default='/workspace/cheXnet_tf_hvd/hvd_saved_model_64_30e',
...
def record_parser(example_serialized, is_training, dtype):
    ...
    # perhaps change to decode_png and resize
...
def input_fn(is_training,
    ...
    print("dataset using %s datatype." %(dtype)) 
...
def chexnet_model(FLAGS):
    ...
    #compression = hvd.Compression.fp16 if FLAGS.use_float16 else hvd.Compression.none
  
 
    if FLAGS.use_float16:
        print("Using horovod float 16 compression")
    ...
    if FLAGS.opt == 'adam':
        ...
        opt = optimizers.SGD(lr=lr, momentum=FLAGS.momentum, nesterov=FLAGS.nesterov)
    ...
    #print("model using %s dtype." % (K.floatx()))
    return model
...
def set_model_precision(FLAGS):
    # setup the model to use float16 if asked
    if FLAGS.use_float16:  
...
def main():
    ...
    # save the final model after all epoch - overwrites existing
    #if hvd.rank() == 0:
    print("time for training: {}s".format(end - start))
    # with open('/mnt/leela/cheXnet_Training/hvd_saved_model/model_architecture.json', 'w') as f:
